...
- **Faberビルドエンジン (beta)** — 内蔵の並列ビルド実行エンジン。`[build]` に `engine = "faber"` を指定すると、Ninja をインストールせずにビルドできる。既定で論理CPUコア数まで並列実行し（`-j` で上書き可能）、タイムスタンプとコンパイラのdepfileからインクリメンタルに再ビルドする。
- **`ordo test` / `ordo check` のFaber対応** — テストバイナリのビルドと構文チェックも同じエンジンを通るため、`engine = "faber"` がこれらの経路もカバーする。
- **Arsenalツールチェインマネージャ (beta)** — `ordo toolchain install | list | which | remove | update | clean` で外部ツールのダウンロードとバージョン管理を行う。Ninja は公式のGitHub Releasesから取得。複数バージョンが併存でき、`[toolchain]` の `ninja = "1.12"` でプロジェクトごとに固定できる。ツール解決は Arsenal 管理のバイナリを優先し、無ければ `PATH` にフォールバックする。
- **Ninjaの自動プロビジョニング** — ビルドにNinjaが必要で見つからない場合、失敗せずにArsenal経由のインストールを提案する。`CI` または `ORDO_YES` が設定されている場合はプロンプトを省略する。
- **ビルドディレクトリロック** — 同じビルドディレクトリに対する2つ目のビルドは、割り込まずに先行ビルドの完了を待つ。

### バグ修正

- **`compile_commands.json` のシステムインクルードパス** — コンパイラのシステムインクルードパスをコンパイルデータベースに記録するようにし、GCC環境でclang-tidyが標準ヘッダを見つけられない問題を解消。
- **リリースバイナリの OpenSSL 依存を解消** — Arsenal のダウンローダを rustls に変更し、配布バイナリが OpenSSL に依存しないようにした。
